...
1. **在仓库根目录整理任务资料**
   ...
   - 将本地原始数据或脱敏数据复制到 `tasks/<任务名>/data/` 中，确保 Copilot CLI 以及容器可访问。
2. **描述需求并校验输入**
   - 在 `tasks/<任务名>/prompt.md` 中写明分析目标、数据位置、关键业务约束及期望输出。
   - 对照 `README.md` 中的项目目标，确认本次任务适合通过 Copilot CLI 自动化执行。
3. **调用 Copilot CLI 生成初版脚本**
   - 于仓库根目录执行 Copilot CLI 命令，引用 `prompt.md` 内容及数据路径。
...
5. **迭代执行与调试**
   ...
   - 若 Copilot CLI 自动修复失败或遇到业务异常，手动更新 `prompt.md` 或脚本，并按序号保留 `script_v02.R`、`run_02.log` 等版本，方便回溯。
